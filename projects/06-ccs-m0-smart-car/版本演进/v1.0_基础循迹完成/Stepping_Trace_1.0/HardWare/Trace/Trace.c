...
unsigned char Ok_falg = 0;
float Read_Grayscale(void)
{
    if(Ok_falg == 2){return 0x0E;}                                   //返回完成结束标志位
    if(wait_stop > 0){
        Stepper_Stop();
        wait_stop --;
        return 0xF;
    ...
    if (count_zeros > 0 && count_zeros < 5)
    {
    ...
    else if(count_zeros > 4 && last_Count_zeros == 0xB && Grayscale_Data != 0x80 && Grayscale_Data != 0xE0 && Grayscale_Data != 0xC0){           // 检测到黑色块
        Black_fast_flag ++;
        Black_fast_flag = Black_fast_flag > 3 ? 1 : Black_fast_flag;
        last_Count_zeros = 0;
        count_zeros_abnormal_Count_Flag = 0;                       // 检测到非长白线将标志位置0
        prevent_coiled_flag = 0;                                   // 防止连续检测黑色块 -- 双重保护
        if(Black_fast_flag == 3){Ok_falg++;}                        // 一次Black_fast_flag == 3 及运行一圈整
    return 0;
    }
    count_zeros_abnormal_Count_Flag = 0;
...
float Grayscale_Pid(float target_value , float Measurements) // 目标值  测量值
{
    ...
    out = steering_Pid.steering_Kp*steering_Pid.steering_Error + steering_Pid.steering_Kd * steering_Pid.steering_Error_difference;
    out = out > 90 ? 90:out;
    out = out < -90 ? -90:out;
    return out;
}
